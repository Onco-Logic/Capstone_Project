...
).fit_transform(X_umap_in)
clusters_umap = KMeans(n_clusters=5, random_state=42).fit_predict(X_umap_in)
plot_step("UMAP", umap_emb[:, :2], clusters_umap, X_umap_in, df["Class"])

dmap_df = pd.DataFrame(
    umap_emb, columns=[f"UMAP_{i+1}" for i in range(umap_emb.shape[1])]
)
...
K_GENES = 350
top_genes = (
    df.drop(columns="Class").var().sort_values(ascending=False).head(K_GENES).index
)
